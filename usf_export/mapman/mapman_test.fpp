...
CCCCMRH ifdef(_ioinit,[      LOGICAL JUNK])dnl
C     ..
C     .. Local Arrays ..
...
C     .. External Functions ..
      INTEGER LENSTR
CCCCMRH ifdef(_ioinit,[
CCCCMRH       LOGICAL IOINIT])dnl
CCCCMRH       EXTERNAL LENSTR
C     ..
C     .. External Subroutines ..
...
C         want to obey format characters in column 1
          ...
          FRM = 'FORMATTED'
CCCCMRH ifdef(_ioinit,
CCCCMRH [      JUNK = IOINIT(.TRUE., .FALSE., .FALSE., ' ' , .FALSE.)
CCCCMRH ])dnl
        ELSE
C         no special significance to column 1
          CCNTRL = 'LIST'
CCCCMRH ifdef(_ioinit,
CCCCMRH [      JUNK = IOINIT(.FALSE., .FALSE., .FALSE., ' ' , .FALSE.)
CCCCMRH ])dnl
        END IF
...
CCCCMRH ifelse(_cant_unlink,1,,[
CCCCMRH       IF (ISTAT.EQ.SCRTCH) CALL CUNLINK (NAMFIL)]
CCCCMRH )dnl
C
C     Error check
 5    CONTINUE
