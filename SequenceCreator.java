...
public class SequenceCreator {
	public static void main(String[] args) {
		for (int i = 1; i <= 1000; i++) {
			...
			String filename = "../../../disk2/bound_perturb/".concat(args[0]).concat("/aa").concat(args[0]).concat(".ppk_{").concat(iString).concat("}kb.pdb");
			BufferedReader br = null;
			PrintWriter out = null;
			try {
				br = new BufferedReader(new InputStreamReader(new FileInputStream(filename)));
				out = new PrintWriter(new BufferedWriter(new FileWriter(filename.concat("2"), true)));
			} catch (Exception e) {
				continue;	
			}
			String print = "";
